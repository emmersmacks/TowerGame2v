...
public class MazeGenerator
{
    ...
    public Maze GenerateMaze(int width, int height)
    {
        ...
        maze.cells[maze.finishPosition.x, maze.finishPosition.y].cellType = PrefabsType.exit;
        return maze;
    }
    ...
    private Vector2Int PlaceMazeExit(MazeGeneratorCell[,] maze)
    {
        ...
        if(maze[furthest.X, furthest.Y].WallBottom == false)
        {
            int count = 1;
            while (maze[furthest.X, furthest.Y - count].WallBottom == false)
            {
                count++;
            }
            furthest.Y = furthest.Y - count;
        }

        //if (furthest.X == 0) furthest.WallLeft = true;
        //else if (furthest.Y == 0) furthest.WallBottom = true;
        //else if (furthest.X == Width - 2) maze[furthest.X + 1, furthest.Y].WallLeft = true;
        //else if (furthest.Y == Height - 2) maze[furthest.X, furthest.Y + 1].WallBottom = true;
